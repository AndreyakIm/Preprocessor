        ...
        private void SAVE()
        {
           /* if(materialComboBox.Items.Count != 0)
                parent.currentFullModel.materials.ListOfMaterials[materialComboBox.SelectedIndex].UpdateMaterial(materialName.Text, 
                    Convert.ToDouble(elasticModulus.Text), Convert.ToDouble(poissonsRatio.Text), Convert.ToDouble(tension.Text), Convert.ToDouble(tension2.Text), Convert.ToDouble(thickness.Text)); */
        }
